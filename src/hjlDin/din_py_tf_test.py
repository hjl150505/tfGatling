...
def tf_record_parser():
    ...
    features = {"user": tf.io.FixedLenFeature(dtype=tf.int64,
                                                      shape=(1,),
                                                      default_value=0),
                "label": tf.io.FixedLenFeature(dtype=tf.int64,
                                              shape=(1,),
                                              default_value=0)}
    featrueSeq = {"seq_feature": tf.io.FixedLenSequenceFeature(shape=(),
                                                                dtype=tf.string,
                                                                allow_missing=True,
                                                                default_value=None)}
...
t = data_gen("E:\\gitWork\\tfGatling\\src\\tfDataSet\\utils\\SeqTfrecord",4)
...
model = tf.keras.models.Model(inputs={**num_inputs}, outputs=[seq_fea_len])
print("name=>" + name)
