...
class DIN(ModelBuilderBase):
    ...
    def build(self):
        ...
        his_seq = input_features['his_item_id']
        ##########################################     修改din
        ...
        for unit in self.hidden_units:
            self.fc.add(keras.layers.Dense(unit, activation=self.att_activation, name="fc_att_" + str(unit)))
        self.fc.add(keras.layers.Dense(1, activation=None, name="fc_att_out"))

        output_mask = K.any(K.not_equal(keys_emb, 0), axis=-1)
        key_masks = tf.expand_dims(output_mask, axis=1)
        keys_len = keys_emb.get_shape()[1]
        queries = K.repeat_elements(query_emb, keys_len, 1)
        deep_input = tf.concat(
            [queries, keys_emb, queries - keys_emb, queries * keys_emb], axis=-1)

        attention_score = self.fc(deep_input)
        outputs = tf.transpose(attention_score, (0, 2, 1))
        ...
        if not self.return_score:
            outputs = tf.matmul(outputs, keys_emb)
...
if __name__=="__main__":
    ...
    if train:
        ...
        reloadModel = keras.models.load_model(".\\modelRsTmp",compile=False)
