...
FeatType = namedtuple('FeatType', ['CatName', 'NumName', 'SeqName'])('Cat', 'Num', 'Seq')
print(FeatType)


class FeatParser(object):
    def __init__(self, rp=None, num_default=0.0, num_dtype=tf.float32, cat_default='', cat_dtype=tf.string,
                 seq_defult='', seq_dtype=tf.string,
                 mean_stddev_path=None):
        self._rp = rp
        self._default_map = defaultdict(lambda: '', {FeatType.CatName: cat_default, FeatType.NumName: num_default,
                                                     FeatType.SeqName: seq_defult})
        self._dtype_map = defaultdict(lambda: tf.string, {FeatType.CatName: cat_dtype, FeatType.NumName: num_dtype,
                                                          FeatType.SeqName: seq_dtype})
    ...
    def parse_feat_config(self):
        ...
        for use, name, type_, size, buckets, weight_fc, mean, stddev, normalize in df.values:
            ...
            default = self._default_map[type_]
            dtype = self._dtype_map[type_]

            if type_ == FeatType.SeqName:
                try:
                    buckets_parsed = int(float(buckets))
                    if buckets_parsed <= 0:
                        raise ValueError('seq buckets less than 0')
                except ValueError as ex:
                    print('[ERROR] seq feature({}) parse error'.format(name))
                    print('[ERROR] >>', ex)
            ...
            if type_ == FeatType.CatName:
                try:
                    buckets_parsed = int(float(buckets))
                    if buckets_parsed <= 0:
                        raise ValueError('buckets less than 0')
                except ValueError as ex:
                    print('[ERROR] categorical feature({}) parse error'.format(name))
                    print('[ERROR] >>', ex)
    ...
    def get_seq_feat(self):
        if self._feat_props is None:
            return None

        return [(use, name, size, buckets, weight_fc, default, dtype)
                for use, name, type_, size, buckets, weight_fc, mean, stddev, normalize, default, dtype
                in self._feat_props if type_ == FeatType.SeqName]
